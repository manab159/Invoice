...
def fetch_data(request):
    ...
    if request.method == 'POST' :
        ...
        try :
            ...
            invoice_obj.total_quantity = Invoice_Line.objects.aggregate(Sum('quantity'))
            invoice_obj.total_amount = Invoice_Line.objects.aggregate(Sum('line_total'))
...
def fetch_invoice_data(request,id):
    ...
    if request.method=='GET' :
        if Invoice.objects.filter(id=id).exists():
            invoice_obj = Invoice.object.get(id=id)
            invoice_json = serializers.serialize('json', invoice_obj)
            return JsonResponse(invoice_json, safe=False)


    if request.method=='PUT' :
        if Invoice_Line.objects.filter(id=id).exists():
            pass
